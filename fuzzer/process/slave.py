...
class SlaveProcess:
    ...
    def execute(self, data, info, need_hits=False):
        self.statistics.event_exec()

        exec_res = self.__execute(data)

        # regurlar하면서 새로운 bitmap이 발견되면 True, regurlar하지 않으면서 새로운 bitmap발견되도 true, 그 이외에는 False
        is_new_input, *hits = self.bitmap_storage.should_send_to_master(exec_res, need_hits=need_hits)
        # exit_reason이 "regular"가 아닌지 체크
        ...
        if is_new_input:
            if not crash:
                ...
                # 해당 payload를 한번 더 돌려보면서 검증한다. funky는 quick_validate 8번 돌려보고 bitmap이 변했는지 체크한다.
        ...
        return exec_res, is_new_input, *hits
