...
class FuzzingStateLogic:
    ...
    def execute(self, payload, label=None, extra_info=None, need_hits=False):
        ...
        parent_info = self.get_parent_info(extra_info)
        bitmap, is_new, *hits = self.slave.execute(payload, parent_info, need_hits)
        # bitmap, is_new, (new_bytes, new_bits)
        if is_new:
